...
class Egany_FB_Group_To_WP {
    ...
    function historical_import() {
		...
		{  
			...
			var_dump('fetch...'. $key);  
			$this->fetch_facebook_data($source_id, $limit, $access_token, $page_num, $comment_max);  
			// break; return; 
		} 
     ...
     * @return none 
     */
    function fetch_facebook_data($source_id, $limit, $access_token, $page_num, $comment_max) {
     ...
     */
    function do_import() {
        $option = $this->get_settings('fb2wp_general');
        ...
        // $access_token = $option['app_id'] . '|' . $option['app_secret'];
		//phong.nguyen 20150416 get token, source 
		$access_token = $this->get_access_token($option);  
        $source_id     = $this->get_source_id($option, 'page');   
        $limit        = isset( $option['limit'] ) ? intval( $option['limit'] ) : 30;
        //$url          = 'https://graph.facebook.com/' . $source_id . '/feed/?limit=' . $limit . '&access_token=' . $access_token;
		$url          = 'https://graph.facebook.com/v2.3/' . $source_id . '/feed/?limit=' . $limit . '&access_token=' . $access_token;
     ...
     * @return int
     */
    function insert_posts( $group_posts, $source_id ) {
        $count = 0;

        if ( $group_posts ) {
            foreach ($group_posts as $fb_post) {
				 
                $post_id = $this->insert_post( $fb_post, $source_id );
                if ( $post_id ) { 
     ...
     * @return int|WP_Error
     */
    function insert_post( $fb_post, $source_id ) {
		...
		if(count($posts) > 0)  
		{
			...
			if($_fb_updated_time_OLD != $fb_post->updated_time )  // can NOT use "post_modified"  and "post_date_gmt"
			{
				...
				// var_dump('wp_update_post...asd01 '. '_fb_updated_time_OLD... ' . $_fb_updated_time_OLD) ;  
			}
